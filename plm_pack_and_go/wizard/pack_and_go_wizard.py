...
class PackAndGo(models.TransientModel):
    ...
    bom_computation = fields.Selection(
        [
            ("ONLY_PRODUCT", "Selected product"),
    ...
    file_name_computation = fields.Selection([
        ...
        File name inside zip folder
          ...
          * File name and revision
          -> <ir_attachment.name>_<ir_attachment.engineering_revision>.exte
          * Product engineering Code
          -> <product_product.engineering_code>.exte
          * Product engineering name and revision
          -> (<product_product.engineering_code>_
              <eproduct_product.engineering_revision>).exte
          * Product engineering name description
          -> <product_product.engineering_code>_<product_product.name>.exte
          * Product engineering name revision and descriprion
          -> (<product_product.engineering_code>_<product_product.engineering_revision>_
             <product_product.name>).exte
          * Product internal reference' - > <product_product.default_code>.exte
          * Product internal reference and description'
          -> <product_product.default_code>_<product_product.name>.exte

      REMARK:
      This function only work for the non CAD native files.
        [es. it work for (pdf,dxf,stp)]
    ...
    def docCheckCreate(self, ir_attachment_id, product_product_id=False):
        ...
        singleCreateDict = {
            "component_id": product_template_id,
    ...
    def action_compute_attachment_bom(self):
        ...
        #
        computed = []
        for product_product_id, attachment_ids in self.get_attachment(
            self.component_id.product_variant_id
        ).items():
            for ir_attachment_id in attachment_ids:
                ...
                if ir_attachment_id.document_type == "2d":
                    ...
                    logging.error(
                        f"Document {ir_attachment_id.name} of type"
                        f" {ir_attachment_id.document_type} not collexted"
                    )
    ...
    def computeExportRelField(self, forceType=False):
        ...
        checkedDocumentIds = (
            []
        )  # To know if the same document has been already analyzed
        ...
        def recursionDocuments(docBrwsList):
            for ir_attachment_id in docBrwsList:
                res = plmDocObject.CheckAllFiles(
                    [ir_attachment_id.id, [], False, "localhost", "pack_go"]
                )  # Get all related documents to root documents
        ...
        recursionDocuments(
            self.component_id.product_variant_id.linkeddocuments
        )  # Check / Create ROOT structure
        for product_template_id in objProduct.browse(
            compIds
        ):  # Check / Create BOM structure
    ...
    def getAllAvailableTypes(self):
        ...
        try:
            ...
            if serverAddress is None:
                paramObj.create(
                    {"key": "plm_convetion_server", "value": "my.servrer.com:5000"}
                )
                serverAddress = 'no_host_in_odoo_parameter"plm_convetion_server"!'
            fileExtensionsRes = requests.get(
                "http://" + serverAddress + "/odooplm/api/v1.0/getAvailableExtention"
            )
    ...
    def computeDocName(self, lineBrws, outZipFile, compute_file_name=True):
        ...
        if compute_file_name:
            ...
            if self.file_name_computation == "FILE_NAME_REV":
                file_name = (
                    f"{file_name_no_exte}_{ir_attachment_id.engineering_revision}{exte}"
                )
            else:
                if product_id:
                    if self.file_name_computation == "PRODUCT_ENGINEERING_CODE":
                        ...
                        self.file_name_computation == "PRODUCT_ENGINEERING_DESCCRIPRION"
                    ):
                        file_name = (
                            f"{product_id.engineering_code}_{product_id.name}{exte}"
                        )
                    elif (
                        self.file_name_computation
                        == "PRODUCT_ENGINEERING_REV_DESCCRIPRION"
                    ):
                        file_name = (
                            f"{product_id.engineering_code}_"
                            f"{product_id.engineering_revision}_"
                            f"{product_id.name}{exte}"
                        )
    ...
    def exportConverted(self, line_wizard_brows):
        ...
        relStr = (
            ...
            .sudo()
            ._get_param("extension_integration_rel")
        )
        try:
            rel = eval(str(relStr).lower())
        except Exception as ex:
            logging.error(
                "Unable to get extension_integration_rel parameter. EX: %r" % (ex)
            )
            rel = {}
        integration = rel.get(str(self.getFileExtension(ir_attachment_id)).lower(), "")
        convertObj = self.env["plm.convert"]
        filePath = convertObj.getFileConverted(
            ir_attachment_id, integration, line_wizard_brows.available_types
        )
    ...
    def action_export_zip(self):
        ...
        try:
            ...
            logging.error(
                "Unable to delete file from export function %r %r"
                % (outZipFile, str(ex))
            )
    ...
    def get_attachment(self, product_product_id, version="latest", bom_type="normal"):
        """
        ...
        if bom_level == "FIRST_LEVEL":
            ...
            ids = product_product_id._getChildrenBom(
                product_product_id, level=bom_level == "FIRST_LEVEL", bom_type=bom_type
            )
            for child_product_product_id in self.get_version_list(
                self.env["product.product"].browse(ids)
            ):
            ...
            product_product_ids = self.env["product.product"]
            for leaf_product_id in self.get_version_list(
                product_product_id.getLeafBom(bom_type=bom_type)
            ):
        ...
        for product_product_id in product_product_ids:
            for ir_attachment_id in product_product_id.linkeddocuments:
                if ir_attachment_id not in all_attchment_collected:
                    ...
                    for ref_ir_attachment_id in ir_attachment.browse(
                        ir_attachment.getRelatedHiTree(
                            ir_attachment_id.id, recursion=True, getRftree=True
                        )
                    ):
