...
def get_parent(my_object):
    return [
        my_object.product_tmpl_id.name,
        "",
...
class ReportSpareDocumentOne(models.AbstractModel):
    ...
    def _create_spare_pdf(self, components):
        ...
        main_book_collector = BookCollector(
            customText=self.get_custom_text(), bottomHeight=10, poolObj=self.sudo().env
        )
        for component in components:
            processed_objs = []
            buf = self.get_first_page([component.id])
            main_book_collector.addPage((buf, ""))
            self.get_spare_parts_pdf_file(
                component,
                main_book_collector,
                component_type,
                bom_type,
    ...
    def get_spare_parts_pdf_file(
        ...
        bom_brws_ids = bom_template.search(
            [("product_id", "=", product.id), ("type", "=", "spbom")]
        )
            ...
            if bom_brws_ids:
                ...
                if len(bom_line_ids) > 0:
                    ...
                    pdf = (
                        ...
                        ._render_qweb_pdf(report_ref, res_ids=bom_brws_ids.ids)[0]
                    )
                    page_stream = BytesIO()
    ...
    def get_first_page(self, ids):
        ...
        pdf = (
            report_ref.sudo()
            .with_context(force_report_rendering=True)
            ._render_qweb_pdf(report_ref.id, res_ids=ids)[0]
        )
    ...
    def _get_report_values(self, doc_ids, data=None):
        ...
        return {"docs": documents, "get_content": self.create_spare_pdf}
