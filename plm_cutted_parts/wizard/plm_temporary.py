...
class PlmTemporaryCutted(models.TransientModel):
    ...
    def _action_on_bom(self, product_id, explosion_action):
        """
        ...
        if product_id.row_material:
            if mrp_bom_ids:
                for bom_line_id in mrp_bom_ids:
                    if (
                        bom_line_id.product_id.row_material.id
                        != product_id.row_material.id
                    ):
            ...
            for mrp_bom_id in mrp_bom_ids:
                for bom_line_brws in mrp_bom_id.bom_line_ids:
                    self._action_on_bom(
                        bom_line_brws.product_id, explosion_action
                    )  # is raw material
                break
    ...
    def cutted_part_action(self, bom_line_brws, explosion_action):
        ...
        if explosion_action == "replace":
            x_len = mrp_bom_line_type_object.computeXLenghtByProduct(
                bom_line_brws.product_id
            )
            y_len = mrp_bom_line_type_object.computeYLenghtByProduct(
                bom_line_brws.product_id
            )
            to_write = {
                "product_id": bom_line_brws.product_id.row_material.id,
                "cutted_type": "none",
                "product_qty": bom_line_brws.computeTotalQty(
                    x_len, y_len, bom_line_brws.product_qty
                ),
            ...
            bom_brws_list = mrp_bom_type_object.search(
                [("product_tmpl_id", "=", id_template), ("type", "=", "normal")]
            )
            ...
            if not bom_brws_list:
                ...
                for bom_brws in bom_brws_list:
                    cuttedLines = bom_brws.bom_line_ids.filtered(
                        lambda line: line.cutted_type == "server"
                    )
                    ...
                    for bom_line_brws in cuttedLines:
                        ...
                        if not bom_line_brws.bom_id.product_id.row_material:
                            bom_product_tmpl_id = bom_line_brws.bom_id.product_tmpl_id
                            raw_mat = (
                                bom_product_tmpl_id.product_variant_id.row_material
                            )
    ...
    def action_create_normalBom(self):
        ...
        for plmTmpObj in self:
            ...
            response = super(
                PlmTemporaryCutted, self.with_context(ctx)
            ).action_create_normalBom()
