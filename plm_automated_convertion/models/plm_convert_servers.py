...
class PlmConvertServers(models.Model):
    ...
    available_format = fields.One2many(
        comodel_name="plm.convert.format",
        inverse_name="server_id",
        string="Available Formats",
    )
    folder_to = fields.Text("Folder To")
    available = fields.Boolean("Is Available", default=True)
            ...
            if server.proc_to_kill and not server._is_internal:
                ...
                try:
                    response = requests.post(
                        url, params=params, timeout=self.getTimeOut()
                    )
                except Exception as ex:
    ...
    def getClientProcesses(self):
        for server in self:
            if not server._is_internal:
                ...
                if response.status_code != 200:
                    raise UserError(
                        "Wrong responce from server %r" % (response.status_code)
                    )
                res = response.json()
                outMsg = ""
    ...
    def updateAvailableFormat(self):
        ...
        try:
            ...
            url = "%s//%s:%s/odooplm/api/v1.0/getAvailableExtention" % (
                self.address,
                self.protocol,
                self.port,
            )
            ...
            for from_format, values in ret.items():
                ...
                for format_to in convertion_to_formats:
                    if not self.available_format.filtered(
                        lambda x: x.start_format == from_format
                        and x.end_format == format_to
                        and x.cad_name == cad_name
                    ):
                        obj_format.sudo().create(
                            {
                                "start_format": from_format,
